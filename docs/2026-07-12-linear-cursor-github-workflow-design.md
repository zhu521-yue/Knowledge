...
- 当前 Milestone 的 Linear 进度必须达到 `100%`；所有属于该 Milestone 的 Parent Issue 和 Sub-issue 都必须进入 `Done`，或经用户确认进入 `Canceled` / `Duplicate` 等终态。
- 阶段端到端验收通过。
- 对应 PR 全部合并。
- 配置说明完整。
- 没有未记录的阻塞项。
- 下一阶段前置条件具备。
- 当前 Milestone 未完成前，不得继续执行下一 Milestone 的新任务；如果下一阶段已有提前完成任务，不回退历史，但暂停继续推进，先补齐当前阶段。
- 用户批准当前 Milestone 启动后，阶段内单个任务完成并通过验证时，Cursor 可以直接提交、推送、创建 PR、合并 PR 并同步 Linear；不再逐任务请求确认。
- 每个 Milestone 完成后，Cursor 必须向用户交付阶段说明，包含：本阶段完成了哪些任务、达到了什么可观察效果、用户如何通过前端验证本阶段实现的功能；用户确认后才进入下一 Milestone。
...
用户对当前 Milestone 的启动批准，授权 Cursor 连续交付该阶段内符合既定范围的 Sub-issue。阶段内单个任务完成并通过自动验证后，Cursor 可以直接 Commit、Push、创建 PR、合并 PR，并同步 Linear 状态，不再逐任务请求用户确认。出现账号、密钥、费用、服务地址、端口、数据目录、隐私授权、重大架构变化或阶段范围扩张时，授权自动暂停并等待用户决策。
...
Milestone 完成时，Cursor 必须额外交付阶段审核包：
...
前端验证必须给出用户可操作、可观察的路径，例如页面地址、按钮、输入数据和期望看到的界面变化。若当前阶段没有用户可见前端功能，必须明确说明原因，并提供最接近用户视角的替代验证方式。
