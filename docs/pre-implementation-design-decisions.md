...
首个管理员初始化已确认（M1 实施配置修订）：

- 首个管理员通过后端一次性 CLI 命令交互创建；API 启动过程不得读取管理员用户名或密码，也不得自动创建账号。
- CLI 只在用户表为空时允许执行；用户表非空时安全拒绝，禁止覆盖管理员密码或创建第二个初始管理员。
- 密码通过无回显交互输入并二次确认，不接受命令行参数、环境变量或 `.env`，避免进入进程列表、Shell 历史、Compose 配置和日志。
- CLI 与 API 复用同一个应用 Use Case、密码策略、Argon2id 校准和事务边界，不允许另写绕过领域规则的初始化 SQL。
- 禁止内置默认密码、空密码或弱密码；失败时只输出不含用户名、密码和哈希的诊断信息。
- 成功后 CLI 立即退出；初始化能力保留用于空数据库恢复，但不能在已有用户的数据库上重复执行。
