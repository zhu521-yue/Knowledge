...
| 数据类别 | 默认路由 | 授权要求 | 禁止时行为 |
| --- | --- | --- | --- |
| 资料最小必要片段、来源引用 | 外部 LLM 或远程 Embedding | 长期授权或本次明确允许 | 依赖模型的任务明确失败 |
| 用户答案、Rubric、判题上下文 | 用户配置的外部 LLM | 数据类别授权 + 任务授权 | 不执行生成或判题 |
| 学习历史摘要、认知画像、情绪内容 | MVP 不外发 | 独立、窄化授权；不得继承资料授权 | 保持本机处理或功能不可用 |
| URL 抓取结果 | 本机抓取与解析 | 提交 URL 只授权该次抓取 | 不自动进入模型上下文 |
| 密码、Token、API Key、内部路径、系统日志 | 禁止外发 | 不存在可覆盖授权 | Adapter 白名单拒绝 |

Provider 边界不变量：

- 配置凭据不等于授权内容发送；每次调用都固化不可变 `consent_snapshot` 并在最终 HTTPS 边界重新校验。
- LLM 不做隐式 fallback；Embedding 只允许“远程 Ollama → 已配置且已授权的外部 API”单层 fallback，且整条 Run 全量重算为独立索引版本。
- 重试、分页、修复请求和 fallback 都是新的外部发送动作，发起前必须重新确认账号、配置和授权仍有效。
- Provider 输入仅含完成当前动作的白名单字段；输出先经过大小、schema、引用和向量维度校验，再进入应用层。
- 超时、限流和第三方错误按 D04/D10/D11 映射；日志与审计不得保存完整 Prompt、正文、答案、请求头、凭据或原始响应。
- 用户只能访问自己的 Provider 配置和调用审计；管理员只可查看脱敏健康信息与聚合指标。
...
| 层级 | 责任 | 必测边界 | 失败含义 |
| --- | --- | --- | --- |
| 纯单元测试 | 领域函数、状态机、预算计算、错误映射 | 无网络、无数据库、确定性输入输出 | 禁止合并对应实现 |
| Adapter 契约测试 | MySQL、Milvus、文件、PDF、LLM/Embedding Adapter | 第三方 schema、超时、限流、脱敏、输入输出上限 | Provider 或基础设施边界不兼容 |
| 集成测试 | Use Case + 真实测试容器 + Outbox/Worker | 事务、幂等、租约、补偿、用户隔离、发布可见性 | 业务链路不可交付 |
| API/SSE 契约测试 | HTTP 与事件协议 | Problem Details、ETag、Operation、重放、断线恢复 | 客户端协议不稳定 |
| 固定数据集端到端测试 | 干净环境中的完整 MVP 闭环 | 导入、抽取、确认、讲解、判题、复习、检索、删除 | Milestone 不可验收 |
| 性能与资源测试 | D13 基线环境 | 延迟、并发、内存、磁盘、超时、恢复 | 不满足 16 GB / 20 GB MVP 预算 |

测试不变量：

- 单元与领域属性测试不得依赖实时 Provider；外部 API 使用录制后脱敏且版本固定的 fixture 或受控 fake。
- Adapter 契约测试可以访问显式测试服务，但不得使用真实用户资料、生产凭据或不可控费用接口。
- 需要数据库语义的事务、锁、唯一约束和迁移测试使用真实 MySQL 测试容器，不用内存替身宣称通过。
- 固定数据集按内容 hash 和预期结果版本化；测试报告记录代码版本、schema 版本、模型标识和运行环境。
- 失败恢复测试必须在明确 checkpoint 注入故障，验证重试、幂等、补偿和暂存不可见；不能只断言最终 HTTP 状态。
- 性能验收只在 D13 的 16 GB Windows、Docker/WSL2 约 10 GB 上限及 20 GB 业务磁盘基线下形成结论；开发机更快不提高冻结预算。
- 每次只改变一个变量并保留原始指标；不以平均值掩盖超时、OOM、磁盘满或未授权外联。
