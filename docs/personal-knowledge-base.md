...
| `status` | string | `queued` / `running` / `validating` / `published` / `failed` / `cancelling` / `cancelled` |
| `checkpoint` | string | Saga 当前检查点 |
...
stateDiagram-v2
  ...
  Running --> Cancelling: 检查点取消
  Parsing --> Compensating: 失败或取消
  Extracting --> Compensating: 失败或取消
  Chunking --> Compensating: 失败或取消
  Embedding --> Compensating: 失败或取消
  Validating --> Compensating: 失败
...
- `error.payload` 必须区分 `retryable` 和 `fatal`。
- Token 只是传输增量，不是最终事实源；完整讲解或评分先持久化，再发送 `session_done`。
